...
public class PolicyHelper {
    ...
    public void deductLeaves(int cause, String leaveType, List<Date> leaveDays,
                             ...
                             HubModuleImpl am) {
        try {
          CommonUtil.log("inside deductLeaves Everything cool here");
...
//            rows!=null?rows[0].getAttribute("EmpType").toString():""
            ...
            while (rows.length > 0) {
                for (int j = 0; j < rows.length; j++) {
                  String type = rows[j].getAttribute("Type")==null?null:rows[j].getAttribute("Type").toString();
                    flag = rows[j].getAttribute("LeaveType") == null ? null : rows[j].getAttribute("LeaveType").toString();
    ...
    private String getJobStatus(String emp_code) {
        ...
        System.out.println("empCode = "+ emp_code);
